...
function strEnc(data, firstKey, secondKey, thirdKey) {
  ...
  if (leng > 0) {
    if (leng < 4) {
      ...
      const iterator = Number.parseInt(leng / 4)
...
function strDec(data, firstKey, secondKey, thirdKey) {
  ...
  const iterator = Number.parseInt(leng / 16)
  let i = 0
  for (i = 0; i < iterator; i++) {
    const tempData = data.substring(i * 16 + 0, i * 16 + 16)
    const strByte = hexToBt64(tempData)
    const intByte = Array.from({ length: 64 })
    let j = 0
    for (j = 0; j < 64; j++) {
      intByte[j] = Number.parseInt(strByte.substring(j, j + 1))
    }
    var decByte
    if (firstKey != null && firstKey != '' && secondKey != null && secondKey != '' && thirdKey != null && thirdKey != '') {
      var tempBt
  ...
  if (leng < 4) {
    ...
    for (i = 0; i < leng; i++) {
      ...
      for (j = 0; j < 16; j++) {
        ...
        bt[16 * i + j] = Number.parseInt(k / pow) % 2
      }
    }
    for (p = leng; p < 4; p++) {
      var k = 0
      for (q = 0; q < 16; q++) {
        ...
        bt[16 * p + q] = Number.parseInt(k / pow) % 2
      }
    }
  }
  else {
...
function enc(dataByte, keyByte) {
  ...
  const ipLeft = Array.from({ length: 32 })
  const ipRight = Array.from({ length: 32 })
  const tempLeft = Array.from({ length: 32 })
  let i = 0; let j = 0; let k = 0; const m = 0; let n = 0
  for (k = 0; k < 32; k++) {
    ipLeft[k] = ipByte[k]
  ...
  const finalData = Array.from({ length: 64 })
  for (i = 0; i < 32; i++) {
...
function initPermute(originalData) {
  const ipByte = Array.from({ length: 64 })
...
function expandPermute(rightData) {
  const epByte = Array.from({ length: 48 })
  for (let i = 0; i < 8; i++) {
    if (i == 0) {
...
function xor(byteOne, byteTwo) {
  const xorByte = Array.from({ length: byteOne.length })
...
function sBoxPermute(expandByte) {
  const sBoxByte = Array.from({ length: 32 })
  let binary = ''
  const s1 = [
  ...
  for (let m = 0; m < 8; m++) {
    ...
    sBoxByte[m * 4 + 0] = Number.parseInt(binary.substring(0, 1))
    sBoxByte[m * 4 + 1] = Number.parseInt(binary.substring(1, 2))
    sBoxByte[m * 4 + 2] = Number.parseInt(binary.substring(2, 3))
    sBoxByte[m * 4 + 3] = Number.parseInt(binary.substring(3, 4))
  }
  return sBoxByte
}

function pPermute(sBoxByte) {
  const pBoxPermute = Array.from({ length: 32 })
...
function finallyPermute(endByte) {
  const fpByte = Array.from({ length: 64 })
  fpByte[0] = endByte[39]
  fpByte[1] = endByte[7]
  fpByte[2] = endByte[47]
  fpByte[3] = endByte[15]
...
function generateKeys(keyByte) {
  const key = Array.from({ length: 56 })
  ...
  for (i = 0; i < 16; i++) {
    ...
    const tempKey = Array.from({ length: 48 })
    tempKey[0] = key[13]
    tempKey[1] = key[16]
    tempKey[2] = key[10]
    tempKey[3] = key[23]
